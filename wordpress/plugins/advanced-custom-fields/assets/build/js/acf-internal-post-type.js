...
(function ($, undefined) {
  ...
  acf.addAction('select2_init', function ($select) {
    ...
    $select.on('select2:open', function () {
      const dropdownClass = name.includes('schema_property') ? 'schema-property-select-results' : 'schema-output-format-select-results';
      $('.' + dropdownClass + ' input.select2-search__field').attr('placeholder', acf.__('Type to search...'));
    });
  });
