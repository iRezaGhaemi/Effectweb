...
(function ($, undefined) {
  var Field = acf.Field.extend({
    ...
    onFocusSearch: function (e, $el) {
      ...
      // Only set searching state if value differs from saved address.
      if ($el.val() !== address) {
    ...
    onKeyupSearch: function (e, $el) {
      ...
      // Update state based on whether value has changed.
      else if ($el.val() !== address) {
        this.setState('searching');
      } else {
        this.setState('default');
      }
...
(function ($, undefined) {
  const Field = acf.Field.extend({
    ...
    setValue(val) {
      ...
      if (newType) {
        this.$tabButton().filter(`[data-unique-tab-key="${newType}"]`).trigger('click');
        const $iconList = this.$(`.acf-icon-list[data-parent-tab="${newType}"]`);
        if ($iconList.length) {
          $iconList.find('.acf-icon-picker-list-icon.active').removeClass('active');
          $iconList.find('input:checked').prop('checked', false);
          const $newIcon = $iconList.find(`.acf-icon-picker-list-icon[data-icon="${newValue}"]`);
          if ($newIcon.length) {
      ...
      if (newType === 'media_library' && newValue && window.wp?.media?.attachment) {
    ...
    alignIconListTabsToCurrentValue(typeAndValue) {
      const $iconLists = this.$('.acf-icon-list').filter(function () {
    ...
    getIconsList(tab) {
      ...
      if (tab === 'dashicons') {
        ...
        icons = Object.entries(iconPickeri10n).map(([key, value]) => {
          return {
            key,
            label: value
          };
    ...
    unselectIcon($iconList) {
      $iconList.find('.acf-icon-picker-list-icon').removeClass('active');
    ...
    onIconRadioFocus(e) {
      ...
      const $newIcon = $parentTab.find('.acf-icon-picker-list-icon[data-icon="' + icon + '"]');
    ...
    onIconSearch(e) {
      ...
      if (filteredIcons.length > 0 || !searchTerm) {
        ...
        $parentTab.find('.acf-icon-list-empty').find('.acf-invalid-icon-list-search-term').text(visualSearchTerm);
    ...
    alignMediaLibraryTabToCurrentValue(typeAndValue) {
      ...
      if (type === 'media_library') {
        ...
        if (previewUrl) {
          ...
          this.$mediaLibraryPreviewImg().attr('src', previewUrl);
        }
...
(function ($, undefined) {
  var Field = acf.Field.extend({
    ...
    setValue: function (val) {
      ...
      // Update the search input and trigger embed preview.
      this.$search().val(val || '');
      if (val) {
        this.maybeSearch();
      } else {
...
(function ($, undefined) {
  var Field = acf.Field.extend({
    ...
    events: {
      ...
      'input input[type="number"]': 'onChange',
      'change input': 'onChange'
    },
    $input: function () {
...
(function ($, undefined) {
  var Field = acf.Field.extend({
    ...
    setValue: function (val) {
      ...
      // Add each value.
      val.forEach(id => {
        // Find the choice element to get the text.
        const $choice = this.$listItem('choices', id);
        const text = $choice.length ? $choice.html() : acf.strEscape(id);
    ...
    newChoice: function (props) {
      ...
      return ['<li>', '<span tabindex="0" data-id="' + id + '" class="acf-rel-item">' + props.text + '</span>', '</li>'].join('');
    },
...
(function ($, undefined) {
  var Field = acf.Field.extend({
    ...
    onKeyDownLabel: function (e, $label) {
      const key = e.which;
      if (key !== 13) {
        return;
      }
      e.preventDefault();
      // Trigger a click on the contained input to preserve existing behavior.
...
(function ($, undefined) {
  ...
  acf.findFields = function (args) {
    ...
    // query
    if (args?.parent && args?.parent?.find) {
  ...
  var fieldsEventManager = new acf.Model({
    ...
    onChange: function (e, $el) {
      ...
      // Fire change_field action.
      const $field = $el ? acf.findClosestField($el) : false;
      if ($field && $field.length) {
        const field = acf.getField($field);
        if (field) {
          acf.doAction('change_field', field);
        }
      }
    }
  });
...
(function ($, undefined) {
  ...
  acf.models.SelectMediaPopup = MediaPopup.extend({
    ...
    customizeFilters: function (toolbar) {
      ...
      // bail early if no filters
      if (!filters) {
        return;
      }

      // WP 7.0 wraps AttachmentFilters in a container view for accessibility.
      // Find the actual AttachmentFilters instance inside the container.
      if (!filters.filters && filters.views) {
        const subviews = filters.views.get() || [];
      ...
      // bail early if filters are still not available
      if (!filters.filters) {
        return;
      }
...
(function ($, undefined) {
  ...
  var Select2_4 = Select2.extend({
    initialize: function () {
      ...
      // multiple - reorder options to match saved selection order
      const allowReorder = options.allowReorder !== false;
      if (options.multiple && allowReorder) {
      ...
      // multiple - enable drag-to-reorder and maintain selection order
      if (options.multiple && allowReorder) {
        // vars
...
(function ($, undefined) {
  acf.tinymce = {
    ...
    initializeTinymce: function (id, args) {
      ...
      init.setup = function (ed) {
        ed.on('change', function (e) {
          ...
          // Anything but attachments, trigger the change still, this will fire on blur instead.
          // This prevents many network requests on attachments on every keystroke.
          // See repeater field for similar logic.
          if (!$textarea.closest('.attachment-info').length) {
            $textarea.trigger('change');
          }
        });

        // Save editor content on blur in attachment modals only.
        ed.on('blur', function (e) {
          // Only trigger save if inside an attachment modal because there's no save button.
          if ($textarea.closest('.attachment-info').length) {
            ed.save();
            $textarea.trigger('change');
          }
    ...
    enableTinymce: function (id) {
       ...
       * - Fixes a bug in older versions (i.e. WP 6.2) of the block editor when switching between "Block" and "Post" tabs.
       * - Only shows the textarea if a TinyMCE instance doesn't exist to prevent duplicate textareas in ACF blocks.
       */
      if (!tinymce.get(id)) {
        $('#' + id).show();
      }
  ...
  var Validator = acf.Model.extend({
    ...
    showErrors: function (location = 'before') {
      ...
      globalErrors.map(function (error) {
        ...
        if (error.action && typeof error.action === 'object') {
          noticeAction = error.action;
        }
      });
      if (errorCount == 1) {
    ...
    customizeEditor: function () {
      ...
      editor.savePost = function (options) {
        ...
        return new Promise(function (resolve, reject) {
          // Serialize ACF field values into the REST request.
          ...
          // Validate Blocks (v2 only, because blockInstances aren't part of v3).
          ...
          // Validate Blocks (v3 only, because hasAcfError was introduced for V3 blocks).
          const blocks = wp.data.select('core/block-editor').getBlocks();
          function checkBlocksForErrors(blocks) {
            const blocksWithErrors = [];
            return new Promise(function (resolve) {
              // Loop through each block and check if it has inner blocks.
              blocks.forEach(block => {
                if (block.innerBlocks.length > 0) {
                  checkBlocksForErrors(block.innerBlocks).then(innerBlocksHaveError => {
                    if (innerBlocksHaveError) {
                      return resolve(true);
                    }
                  });
                }

                // Check if it has the hasAcfError attribute and it is set to true.
                if (block.attributes.hasAcfError) {
                  const prePublishPanel = document.getElementsByClassName('editor-post-publish-panel')[0];
                  if (prePublishPanel) {
                    wp.data.dispatch('core/editor').togglePublishSidebar();
                  }
                  blocksWithErrors.push(block);

                  // Trigger a custom event to show the error message.
                  document.dispatchEvent(new CustomEvent("acf/block/has-error", {
                    detail: {
                      acfBlockWithValidationErrors: block.clientId
                    }
                  }));
                  acf.debug('Rejecting save because the block editor has a invalid ACF block selected.');
                  return resolve(true);
              ...
              if (blocksWithErrors.length > 0) {
                // Select the first block with a validation error.
                const clientId = blocksWithErrors[0].clientId;
                wp.data.dispatch('core/block-editor').selectBlock(clientId);
              }
              return resolve(false);
            });
          }
          checkBlocksForErrors(blocks).then(blocksHaveError => {
            if (blocksHaveError) {
              notices.createErrorNotice(acf.__('An ACF Block on this page requires attention before you can save.'), {
          ...
          var valid = acf.validateForm({
            ...
            failure: function ($form, validator) {
              ...
              var action = notice.get('action');

              // Check if notice has an action key which is an object with label and url keys.
